...
class Reservations extends Controller
{
	...
	function viewPendingReservations()
	{
		...
		$rooms        = $this->REM->getReservationRoom(null, null, null);
		$guests       = $this->GM->getInfo('GUEST',      null,          null,  null,   null, null, null);
		$reservations = $this->GM->getInfo('RESERVATION','CHECK_IN >=', $date, $order, null, null, 1);
	...
	{
		$room                  = $this->REM->getReservationRoom('ID_RESERVATION', $reservation_id, null);
		$reservationRoomsCount = $this->GM->getCount('ROOM_RESERVATION', 'FK_ID_RESERVATION', $reservation_id, null, null);
		$reservation           = $this->GM->getInfo('RESERVATION',      'ID_RESERVATION',    $reservation_id, null, null, null, 1);
        $reservationRoomInfo   = $this->GM->getInfo('ROOM_RESERVATION', 'FK_ID_RESERVATION', $reservation_id, null, null, null, 1);
		...
		foreach ($reservation as $row)
		{
			...
			$ciDate = $check_in_array[0];
	
			$checkOut_array = explode (' ',$checkOut);
			$coDate = $check_out_array[0];
	...
	function cancelReservation($reservationId, $guestId)
	{
		...
		$guest            = $this->GM->getInfo('GUEST',           'ID_GUEST',    $guest_id, null,   null, null, 1);
		$reservations     = $this->GM->getInfo('RESERVATION',     'FK_ID_GUEST', $guest_id, $order, null, null, 1);
	...
	function createReservation2($roomId)
	{
		...
		$roomInfo     = $this->GM->getInfo('ROOM',      'ID_ROOM',      $room_id,   null, null, null, 1);
		$roomTypeInfo = $this->GM->getInfo('ROOM_TYPE', 'ID_ROOM_TYPE', $room_type, null, null, null, 1);
