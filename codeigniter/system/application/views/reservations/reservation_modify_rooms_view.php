...
{
	if ($row['ID_RESERVATION'] == $reservation_id)
	{	
...
if ($available_type)
{
	...
	{?>
		<a href="<?php echo base_url().'reservations/modifyReservationRooms2/'.$reservation_id.'/'.$room_id.'/'.$row['ID_ROOM']; ?>" onClick="return confirm('Seguro que desea cambiar?')" ><?php echo '# '.$row['NUMBER'];?></a><?php
	}
...
if ($available_other)	
{
	...
	foreach ($room_types as $row)
	{
		...
		foreach ($available_other as $row1)
		{
			...
			{
			?>&nbsp;<a href="<?php echo base_url().'reservations/modifyReservationRooms2/'.$reservation_id.'/'.$room_id.'/'.$row1['ID_ROOM']; ?>" onClick="return confirm('Seguro que desea cambiar?')" ><?php echo '# '.$row1['NUMBER'];?> </a>&nbsp;<?php
			}
		}
 	}
}
...
<p><a href="<?php echo base_url().'reservations/infoReservation/'.$reservation_id?>">Volver</a></p>
